...
int main(int argc, char *argv[])
{
    ...
    MDB_dbi history;
    rc = mdb_dbi_open(txn, "history_db", MDB_CREATE, &history);
    if (rc != 0)
    {
    ...
    cout << "History storage : " << endl;
    MDB_cursor *cursor;
    rc = mdb_cursor_open(txn, history, &cursor);
    if (rc != 0)
    {
    ...
    while (mdb_cursor_get(cursor, &key, &value, MDB_NEXT) == 0)
    {
        ...
        if (csv_log)
        {
            ...
            ofstream myfile;
            myfile.open("../History.csv", ios::out | ios::app);
